...
> **SELESAI 2026-08-04** — lihat SESSION-2026-08-04. Biometrik terverifikasi penuh di Android fisik.
...
- [x] **Verifikasi**: Login biometrik → sidik jari → masuk app tanpa input password ✅ (2026-08-04, Android fisik: enable → logout → login sidik jari berhasil)
- [x] **Verifikasi**: Toggle Off biometrik → entry Keychain terhapus ✅ (2026-08-04: toggle OFF → tombol biometrik hilang dari layar login)
- [x] **Verifikasi**: `REFRESH_REVOKED` → biometrik nonaktif otomatis, arah ke login ✅ (2026-08-04: sesi device di-revoke dari server — DEL Redis key `refresh:{uid}:3b0a9a07...` + SREM registry + `UPDATE user_sessions SET revoked_at` untuk jti `ffae1375...` → login biometrik menampilkan "Sesi biometrik telah berakhir. Silakan login dengan kata sandi." + biometrik nonaktif otomatis)
