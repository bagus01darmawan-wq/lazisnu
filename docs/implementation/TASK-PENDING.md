...
| D10 | 04/05: Redis key per-device, OTP E2E, biometrik login/toggle/REFRESH_REVOKED | 04/05 — **SEBAGIAN SELESAI 2026-08-04** (D10-A + MF3a): format key `refresh:{uid}:{did}` + registry SET + TTL 365d terkonfirmasi di Redis production (`lazisnu-redis-1`, 2 user) dan staging; login ulang device sama → 1 key (overwrite jti) ✅; key per-device dibuat setelah login ✅ (WB2/MA3 level API); **OTP E2E staging SELESAI** ✅ (Fonnte aktif, kode 640875 terkirim ke WA == Redis, verify → token; device Fonnte sempat disconnected → reconnect di dashboard). ⏳ Tersisa (D10-C, butuh perangkat fisik): biometrik di Android fisik (login/toggle/REFRESH_REVOKED) — butuh build APK preview (`eas build -p android --profile preview`) + HP dengan fingerprint. Temuan: `user_sessions.device_id` tidak pernah diisi (`createSession` di `sessionService.ts` tidak menerima deviceId) |
