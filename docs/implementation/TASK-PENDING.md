...
| D3 | 07-D2-4: Verifikasi DSN backend aktif di Sentry dashboard | 07 — ✅ **SELESAI 2026-08-04** (Sentry → Rollbar + Discord via Hookdeck). Code Rollbar live di production (v1.0.2). **Webhook Notification Rollbar → Discord SELESAI 2026-08-04**: user set 6 rules (New Item, Reactivated, 10^nth Occurrence, Resolved, Reopened — level>=error; Deploy — environment==production) → URL `https://hkdk.events/4wdr88dwy46kj8`; verified E2E via test item API (uuid `04249e52...`) → embed `🔴 🆕 NEW ITEM` masuk Discord channel Rollbar. Hookdeck transform `rollbar-to-discord` handle 8 event. Sentry project idle (tidak terpakai); `@sentry/node` masih dipertahankan (dual fail-safe) |
...
| D10 | 04/05: Redis key per-device, OTP E2E, biometrik login/toggle/REFRESH_REVOKED | 04/05 — ✅ **SELESAI 2026-08-04** — D10-A (Redis per-device, WB2/MA3) + MF3a (OTP E2E via Fonnte) + D10-C (biometrik 3 skenario di Android fisik: login sidik jari ✅, toggle off hapus Keychain ✅, REFRESH_REVOKED nonaktif otomatis ✅). PR terkait: #32 phone normalization, #33 keychain v10, #34 logout-biometrik, #36 device_id |
