...
| D10 | 04/05: Redis key per-device, OTP E2E, biometrik login/toggle/REFRESH_REVOKED | 04/05 — **SEBAGIAN SELESAI 2026-08-04** (D10-A + MF3a + biometrik enable): format key `refresh:{uid}:{did}` + registry SET + TTL 365d terkonfirmasi di Redis production & staging; login ulang device sama → 1 key (overwrite jti) ✅; **OTP E2E staging SELESAI** ✅ (Fonnte aktif, kode 640875 WA == Redis, verify → token); **toggle biometrik ON sudah bekerja** ✅ setelah upgrade `react-native-keychain` 8.2.0 → 10.0.0 (PR #33; sebelumnya gagal tanpa prompt di Android); **logout + login biometrik** difix (PR #34 — logout lokal tanpa revoke server saat biometrik aktif). ⏳ Tersisa (D10-C, butuh APK build via EAS `internal-prod`): verifikasi 3 skenario di HP fisik — S1 login sidik jari setelah logout, S2 toggle OFF hapus Keychain, S3 REFRESH_REVOKED (revoke sesi dari server → biometrik nonaktif otomatis; runbook siap) |
