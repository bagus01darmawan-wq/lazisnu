...
|----|------|--------|--------|:---:|
| TD-01 | `auth.integration.test.ts` — mock db global bocor antar describe block | ~~Sudah difix (Sesi 30 Lanjutan 11): `jest.clearAllMocks()` diganti `jest.resetAllMocks()` + helper `setupDefaultDbMocks()` di 6 beforeEach. Mock bocor sudah deterministik, 3 failure yang muncul adalah pre-existing test design issues (TD-04, TD-05).~~ | ~~Test integration gagal setelah run pertama~~ Resolved | ~~Rendah~~ SELESAI |
| TD-02 | `collectionSubmission.integration.test.ts` — test tidak idempotent | `beforeAll` insert district dengan `code: 'DTI'` tanpa cleanup idempotent. Sudah difix dengan cleanup di awal `beforeAll` (sesi ini). 4 file SQL legacy orphan di `migrations/` sudah dihapus (commit `b7e21aa` — Sesi 30 01:52). | ~~Test integration gagal setelah run pertama~~ Resolved | ~~Rendah~~ SELESAI |
| TD-03 | Database test `lazisnu_test` tidak auto-sync dengan Drizzle schema | ~~Sudah difix (Sesi 30 Lanjutan 11): script `db:reset-test` (package.json) + `apps/backend/scripts/db-sync-test.sh` — drop schema + apply migrations via `drizzle-kit migrate` (lebih reliable dari push). Idempotent, bisa dipakai CI.~~ | ~~Test integration bisa gagal setelah schema berubah~~ Resolved | ~~Rendah~~ SELESAI |
| TD-04 | `request-otp` test mocks wrong API | ~~Sudah difix (Sesi 31 Lanjutan): test "should return 200 anti-enumeration" di-rewrite jadi "should return 404 when phone is not registered (TD-04)" — mock target dikoreksi ke `db.query.officers.findFirst` (fungsi yang dipakai route), ekspektasi disesuaikan ke 404 USER_NOT_FOUND (route memang TIDAK anti-enumeration by design — auth.ts:249-251 eksplisit return 404 jika officer tidak ada).~~ | ~~1 test fails~~ Resolved | ~~Rendah~~ SELESAI |
| TD-05 | Session Management test kena rate limit 429 | ~~Sudah difix (Sesi 31 Lanjutan): `resetApp()` baru di `app-helper.ts` — close + rebuild Fastify instance untuk reset @fastify/rate-limit LocalStore (in-memory counter). Dipanggil di `beforeEach` Session Management + `beforeAll` Rate Limit. Counter rate limit kembali ke 0 antar test, sehingga masing-masing test dapat jatah penuh 5 login.~~ | ~~2 test fails (Session Management - rate limit)~~ Resolved | ~~Rendah~~ SELESAI |
| TD-06 | Noise Redis di jest — BullMQ tidak mengenali ioredis-mock | `config/redis.ts` benar memakai ioredis-mock saat test (REDIS_URL tidak diset di .env.test), tapi `services/queues.ts:5` membuat `new Queue('whatsapp-notifications', { connection: redisConnection })` saat import — BullMQ menolak instance ioredis-mock dan membuka koneksi sendiri ke default `127.0.0.1:6379` → 17× `ECONNREFUSED` + warning "Force exiting Jest" di log (`forceExit: true` di jest.config.js memang disengaja, bukan bug). Kena 5 test file: cors.integration, canAccess, taskMetrics, stats-range, collectionSubmission.integration. Ditemukan saat review C1-T0 (20 Sep 2026). Kandidat fix: lazy-init Queue/Worker saat NODE_ENV=test, atau `jest.mock('../../services/queues')` di test terkait — jangan digabung ke commit fitur karena menyentuh jalur queue produksi. | Log test berisik — menyulitkan membaca kegagalan nyata; semua test tetap hijau (36 suite/322 test) | Rendah |