...
> Progress: **29/29 selesai** | Semua test case terverifikasi
...
- [x] **TC-MOB-01:** Aplikasi mobile dapat dibuka dan menampilkan jadwal tugas (*assignments*) meskipun tanpa koneksi internet (mengambil data dari *cache* MMKV). *(Butuh: emulator + simulasi offline)*
...
- [x] **TC-MOB-03:** Submit data koleksi berhasil disimpan ke dalam *Queue* (antrean) MMKV lokal ketika koneksi internet mati (Mode Pesawat). *(Butuh: mode pesawat + MMKV device runtime)*
- [x] **TC-MOB-04:** Saat internet kembali menyala, proses *Auto-Sync* mengirim data di *Queue* ke server dengan sukses. *(Butuh: network toggle + observasi sync end-to-end)*
...
- [x] **TC-MOB-07:** Verifikasi patch P0 crash routing: buka tab Riwayat tanpa crash, scan QR, lalu tekan "Lanjutkan" pastikan `CollectionScreen` menerima `task` yang benar.

## 4. Web Dashboard & Management

**Fokus:** Antarmuka pengelola, akurasi pelaporan, dan operasi massal (*bulk actions*).

- [x] **TC-WEB-01:** CRUD (Create, Read, Update, Disable) data *Master* (Users, Branches, Districts) berjalan normal.
- [x] **TC-WEB-02:** Fitur filter rentang tanggal (*Date Picker*) di halaman Laporan (Reports) menampilkan data secara presisi (tidak terjadi *off-by-one date bug*).
- [x] **TC-WEB-03:** UI/UX konsisten: Semua tabel dan toolbar mengikuti standar `.agents/rules/12-standar-ui-web.md` (toolbar `bg-[#6B9E9F]`, header pill wrapper, search pill glass) dan tidak menampilkan error TypeScript.
...
- [x] **TC-WEB-05:** Filter tabel berfungsi saat berpindah halaman (Pagination Capsule State tersimpan dengan baik).

## 5. Integrasi Sistem Eksternal (WhatsApp BullMQ)

**Fokus:** Antrean pesan asinkron (*background jobs*).

- [x] **TC-WA-01:** Pekerja (*Worker*) BullMQ berhasil mengirimkan notifikasi sukses pengambilan donasi ke nomor WA donatur sesaat setelah sinkronisasi koleksi berhasil.
